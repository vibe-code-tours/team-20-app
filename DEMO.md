...
The hardest part of this project was keeping the role-based flows and the order lifecycle consistent across the whole app. We used OpenAI API support for the chatbot and order extraction pieces, and we kept order handling transactional so stock and order state stay in sync. We also added payment screenshot upload support, which helps organizers verify offline payments without losing the order trail.
